...
export default class Utils {
	...
	async sendCommandWithCheck(command, uuid) {
		...
		if (Buffer.byteLength(command, "utf8") > COMMAND_MAX_LENGTH) throw new Error("命令长度过长");

		// 命令转发限流：超出窗口上限时统一延后到下一个游戏刻发送
		const rl = (cmdCfg.rateLimit) || {};
		const RL_WINDOW_MS = (rl.windowMs !== undefined && rl.windowMs > 0) ? rl.windowMs : 50;
		const RL_MAX = (rl.maxPerWindow !== undefined && rl.maxPerWindow > 0) ? rl.maxPerWindow : 1024;
		if (rl.enabled !== false) {
			const now = Date.now();
			if (!this._rlState) this._rlState = { windowStart: now, count: 0 };
			const st = this._rlState;
			if (now - st.windowStart >= RL_WINDOW_MS) {
				st.windowStart = now;
				st.count = 0;
			}
			if (st.count >= RL_MAX) {
				const nextTick = Math.ceil(now / RL_WINDOW_MS) * RL_WINDOW_MS;
	...
	tell(msg, current = "@a", isPrefix = true) {
		...
		const cmdCfg = (config.command) || {};
		const maxCmd = (cmdCfg.maxLength !== undefined) ? cmdCfg.maxLength : 461;
		let start = 0;
	...
	removeOwner(owner) {
		...
		for (const [event, callback] of subs) {
			...
			if (list) {
				...
				if (list.length === 0) {
					this.subscribeBack.delete(event);
					this.subscribedEvents.delete(event);
				}
			}
		}
