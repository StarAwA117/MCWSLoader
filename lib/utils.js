...
export default class Utils {
	...
	async sendCommandWithCheck(command, uuid) {
		...
		const COMMAND_MAX_LENGTH = (config.safety && config.safety.commandMaxLength !== undefined) ? config.safety.commandMaxLength : 461;
	...
	tell(msg, current = "@a", isPrefix = true) {
		...
		const maxCmd = (config.safety && config.safety.commandMaxLength !== undefined) ? config.safety.commandMaxLength : 461;
		let start = 0;
