...
import { installDependency } from "./modinstall.js";
...
// 启动时检查所有模组的 dependencies，缺失且有 url 的自动安装
export async function checkModDependencies() {
	...
	for (const entry of fs.readdirSync(MOD_DIR, { withFileTypes: true })) {
		...
		for (const dep of deps) {
			try {
				const r = await installDependency(dep, MOD_DIR);
				if (!r.skipped) logger.info(`Dependency "${r.name}" installed for mod "${label}"`);
			} catch (e) {
				logger.error(`Failed to install dependency for mod "${label}": ${e.message}`);
			}
		}
	}
}
