...
export async function checkModDependencies() {
	const checkEnabled = (config.ws && config.ws.checkDependenciesOnLoad !== false);
	if (!checkEnabled) return;
	const modDir = MOD_DIR;
	const entries = fs.readdirSync(modDir, { withFileTypes: true }).filter(e => e.isDirectory());
	for (const entry of entries) {
		const manifestPath = path.join(modDir, entry.name, "manifest.json");
		if (!fs.existsSync(manifestPath)) continue;
		try {
			const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf-8"));
			...
			for (const dep of deps) {
				...
				if (!depName) continue;
				const depPath = path.join(modDir, depName);
				if (fs.existsSync(depPath)) continue;
				if (!dep.url) {
					logger.warning(`Missing dependency ${depName} for ${entry.name} and no download URL provided`);
					continue;
				}
				try {
					logger.info(`Downloading missing dependency: ${depName} from ${dep.url}`);
					const res = await fetch(dep.url);
					if (!res.ok) throw new Error(`HTTP ${res.status}`);
					const buf = Buffer.from(await res.arrayBuffer());
					fs.writeFileSync(depPath, buf);
					logger.info(`Dependency ${depName} downloaded successfully`);
				} catch (e) {
					logger.error(`Failed to download dependency ${depName} for ${entry.name}: ${e.message}`);
				}
			}
		} catch (e) {
			logger.error(`Failed to check dependencies for ${entry.name}: ${e.message}`);
		}
	}
}
