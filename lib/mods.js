...
export class ClientModManager {
	...
	message() {
		this.client.subscribe("PlayerMessage", async (data) => {
			...
			if (permission < 1) { this.client.tell(`§cCommand | §fError > §iunknown command ${msg.split(" ")[0]}`, sender); return; }
			if (!this.execute(sender, msg, this.commands.user)) return;
			if (permission < 2) { this.client.tell(`§cCommand | §fError > §iunknown command ${msg.split(" ")[0]}`, sender); return; }
			if (!this.execute(sender, msg, this.commands.op)) return;
			if (permission < 3) { this.client.tell(`§cCommand | §fError > §iunknown command ${msg.split(" ")[0]}`, sender); return; }
			if (!this.execute(sender, msg, this.commands.owner)) return;
			this.client.tell(`§cCommand | §fError > §iunknown command ${msg.split(" ")[0]}`, sender);
		});
	}
